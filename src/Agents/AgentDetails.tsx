import { ArrowLeft, ArrowSquareOut, Check, Clock, Copy, Info, Lightning, Target, Users } from "@phosphor-icons/react";
...
function getCentroid(bioregionId: string): [number, number] {
  ...
  const prefixMatch = Object.entries(BIOREGION_CENTROIDS).find(([k]) => k.startsWith(prefix));
  if (prefixMatch) {
    // Offset slightly for different regions with same prefix
    const num = Number.parseInt(bioregionId.split("_")[1] || "1", 10);
...
export default function AgentDetails() {
  ...
  const avgLng = commitmentPositions.reduce((sum, p) => sum + p[0], 0) / commitmentPositions.length;
  const avgLat = commitmentPositions.reduce((sum, p) => sum + p[1], 0) / commitmentPositions.length;
          ...
          <div className="grid lg:grid-cols-[480px_1fr] md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <article
                ...
                aria-label={`Profile of ${agent.name || "Agent"}`}
              >
                {/* Header */}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h1 className="text-xl font-semibold">{agent.name || "Unnamed Agent"}</h1>
                ...
                {/* Stats Grid */}
                <div
                  className="p-5 grid grid-cols-4 gap-4 border-b border-gray-100"
                  role="group"
                  aria-label="Agent statistics"
                >
                  <div className="text-center" title="Total ESV tokens earned by this agent">
                    <span className="text-xl font-semibold text-gray-900">{formatNumber(agent.totalESVEarned)}</span>
                  ...
                  <div className="text-center" title="ESV tokens currently staked for bioregion commitments">
                    <span className="text-xl font-semibold text-gray-900">{formatNumber(agent.esvStaked)}</span>
                    <span className="block text-xs text-gray-500">ESV Staked</span>
                  </div>
                  <div
                    className="text-center"
                    title="Reputation score based on completed actions and verification history"
                  >
                    <span className="text-xl font-semibold text-gray-900">{agent.reputationScore}%</span>
                  ...
                  <div className="text-center" title="Number of successful actions completed">
                    <span className="text-xl font-semibold text-gray-900">{agent.actionsCompleted}</span>
                  ...
                  <div className="space-y-2">
                    {agent.commitments.map((c) => (
                      <div key={c.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        <div>
                          <span className="font-medium text-gray-900">{c.bioregionId}</span>
                          {c.locationProofCID && (
                            <a
            ...
            <div className="space-y-4">
              ...
              <div className="h-[300px] rounded-xl overflow-hidden">
                ...
                >
                  {commitmentPositions.map((pos, i) => (
                    <Marker key={agent.commitments[i].id} latitude={pos[1]} longitude={pos[0]} color="#10b981" />
                  ))}
                </MapBox>
              </div>
                          ...
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-xs text-gray-500">{entry.bioregionId}</span>
                                {entry.txHash && (
                                  <a
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white rounded-lg border border-gray-200 p-4 text-center">
                  <span className="text-2xl font-bold text-gray-900">{agent.proposalsSubmitted}</span>
                  <span className="block text-xs text-gray-500 mt-1">Proposals</span>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-4 text-center">
                  <span className="text-2xl font-bold text-gray-900">{agent.bountiesCompleted}</span>
                  <span className="block text-xs text-gray-500 mt-1">Bounties</span>
                </div>
                <div className="bg-white rounded-lg border border-gray-200 p-4 text-center">
                  <span className="text-2xl font-bold text-gray-900">{agent.yieldDelegation || 0}%</span>
                  <span className="block text-xs text-gray-500 mt-1">Yield Delegation</span>
                </div>
              </div>
            </div>
          </div>
