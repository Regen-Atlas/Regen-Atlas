...
import { CELO_TOKENS_MAP, TOKEN_POOL_TOKEN_MAP, UniswapTrading } from "../modules/uniswap";
      ...
      {open && (
        <div className="mt-3 space-y-4">
          <p className="text-xs text-gray-500">
            Independently verified ecosystem service data, valued using scientific methodologies and archived onchain
            for transparency.
          </p>
          {provenances.map((p, i) => {
            const m = p.impact.metrics;
            return (
              <div key={p.pieceCid ?? i} className="bg-grayButton rounded-lg p-4 text-sm space-y-3">
                {/* Header: source + crediting pathway */}
                <div className="flex justify-between items-start">
                  <div>
                    <span className="font-bold capitalize text-base">{p.source.protocol}</span>
                  ...
                  <span className="text-xs text-gray-400">Attested {new Date(p.attestedAt).toLocaleDateString()}</span>
                </div>
                ...
                <div className="text-gray-600 text-xs space-y-0.5">
                  {p.origin.project && (
                    <div>
                      <span className="font-medium text-gray-700">Project:</span> {p.origin.project}
                    </div>
                  )}
                  {p.origin.developer && (
                    <div>
                      <span className="font-medium text-gray-700">Developer:</span> {p.origin.developer}
                    </div>
                  )}
                  {p.origin.methodology && (
                    <div>
                      <span className="font-medium text-gray-700">Methodology:</span> {p.origin.methodology}
                    </div>
                  )}
                  {(p.origin.startDate || p.origin.endDate) && (
                    <div>
                      <span className="font-medium text-gray-700">Period:</span> {p.origin.startDate ?? "—"} to{" "}
                      {p.origin.endDate ?? "present"}
                    </div>
                  )}
                  {p.origin.location?.jurisdiction && (
                    <div>
                      <span className="font-medium text-gray-700">Jurisdiction:</span> {p.origin.location.jurisdiction}
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-gray-200 pt-2">
                  <div className="font-medium text-gray-700 text-xs mb-1">Ecosystem Impact</div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs">
                    {m.climate && (
                      <>
                        ...
                        <div>
                          <span className="text-gray-500">Standard:</span> {m.climate.standard || "—"}
                        </div>
                        {m.climate.vintage && (
                          <div>
                            <span className="text-gray-500">Vintage:</span> {m.climate.vintage}
                          </div>
                        )}
                        {m.climate.methodology && (
                          <div>
                            <span className="text-gray-500">Method:</span> {m.climate.methodology}
                          </div>
                        )}
                      </>
                    )}
                    {m.biodiversity && (
                      <>
                        ...
                        <div>
                          <span className="text-gray-500">Biome:</span> {m.biodiversity.biome}
                        </div>
                        {m.biodiversity.creditType && (
                          <div className="col-span-2">
                            <span className="text-gray-500">Credit type:</span> {m.biodiversity.creditType}
                          </div>
                        )}
                      </>
                    )}
                    {m.energy && (
                      <>
                        ...
                        <div>
                          <span className="text-gray-500">Source:</span> {m.energy.sourceType}
                        </div>
                        {m.energy.farmCount > 0 && (
                          <div>
                            <span className="text-gray-500">Farms:</span> {m.energy.farmCount}
                          </div>
                        )}
                      </>
                    )}
                    {m.marine && (
                      <>
                        ...
                        <div>
                          <span className="text-gray-500">Type:</span> {m.marine.stewardshipType}
                        </div>
                      </>
                    )}
                  </div>
                ...
                <div className="border-t border-gray-200 pt-2">
                  <div className="font-medium text-gray-700 text-xs mb-1">
                    ...
                    <span className="font-normal text-gray-400 ml-1">({p.valuation.methodology})</span>
                  </div>
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
                    <div>
                      <span className="text-gray-500">Total value:</span>{" "}
                      <span className="font-semibold">
                        {formatUSD(p.valuation.totalValue.low)} &ndash; {formatUSD(p.valuation.totalValue.high)}
                      </span>
                    </div>
                    {p.valuation.valuePerUnit && (
                      <div>
                        <span className="text-gray-500">Per unit:</span> ${p.valuation.valuePerUnit.low.toFixed(2)}{" "}
                        &ndash; ${p.valuation.valuePerUnit.high.toFixed(2)} / {p.valuation.valuePerUnit.unit}
                      </div>
                    )}
                  </div>
                  {p.valuation.gapFactor && (
                    <div className="mt-1 text-xs">
                      ...
                      <span className="text-primary-300 font-semibold">
                        {p.valuation.gapFactor.low.toFixed(1)}x &ndash; {p.valuation.gapFactor.high.toFixed(1)}x
                      </span>
                      <span className="text-gray-400 ml-1">(scientific value vs. market price)</span>
                    </div>
                  )}
                  {p.valuation.tokenMarketContext && (
                    <div className="mt-1 text-xs text-gray-500">
                      Token price: ${p.valuation.tokenMarketContext.price.toFixed(4)} (MCap{" "}
                      {formatUSD(p.valuation.tokenMarketContext.marketCap)})
                      <span className="ml-1 text-gray-400">via {p.valuation.tokenMarketContext.source}</span>
                    </div>
                  )}
                </div>
                ...
                {p.pieceCid && !p.pieceCid.startsWith("local:") && (
                  <div className="border-t border-gray-200 pt-2">
                    <div className="text-xs text-gray-500 mb-0.5">Archived on Filecoin (Calibration Testnet)</div>
                    <a
...
export default (): React.ReactElement => {
  ...
  const { item: asset } = useSupabaseItemById<Asset>("assets_published_view", assetId);
  ...
  const { items: fullParentAssets } = useSupabaseItemsByIds<Asset>("assets_published_view", parentAssetIds);
  ...
  const tokenIn = celoContractAddress ? TOKEN_POOL_TOKEN_MAP[celoContractAddress] : "";
  const tokenOut = celoContractAddress ? CELO_TOKENS_MAP[celoContractAddress] : "";

  const celoRetireWalletAddress: Address = asset?.metadata?.celo_retire_wallet_address as Address;

  const provenances = useMemo(() => (assetId ? getProvenancesForAsset(assetId) : []), [assetId]);
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={`${asset.description.substring(0, 160)}...`} />
      </Helmet>
      <Header />
          ...
          <div className="grid lg:grid-cols-[440px_1fr] gap-4">
            ...
            <div>
              <MapBox
                mapStyle={mapStyle}
                initialViewState={{
                  longitude: asset.second_order ? 15 : asset.coordinates.longitude,
                  latitude: asset.second_order ? 30 : asset.coordinates.latitude,
