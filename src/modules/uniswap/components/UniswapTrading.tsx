import { ArrowsDownUp, CircleNotch, Cube, WarningDiamond } from "@phosphor-icons/react";
import { Percent, type Token } from "@uniswap/sdk-core";
import { Route, type SwapOptions } from "@uniswap/v3-sdk";
...
import { divideBigInts, formatNumber, parseNumber } from "../../../shared/helpers";
...
export const UniswapTrading: React.FC<UniswapTradingProps> = ({ tokenPair }) => {
  ...
  const [tradeType, setTradeType] = useState<"exactInput" | "exactOutput">("exactInput");
  ...
  const [status, setStatus] = useState<SwappingState>(!address ? "connect_wallet" : "enter_amount");
  ...
  const handleAmountInChange = async (value: string) => {
    ...
    const pricePerToken = divideBigInts(parseNumber(value, tokenOut.decimals), quote, 5);
  ...
  const handleAmountOutChange = async (value: string) => {
    ...
    const pricePerToken = divideBigInts(quote, parseNumber(value, tokenIn.decimals), 5);
    setSwapExchangeRate(pricePerToken);
  ...
  const handleBuy = async () => {
    ...
    const amountInWithDecimals = parseUnits(amountIn.toString(), tokenIn.decimals);

    try {
      const tradeOptions: SwapOptions = {
        ...options,
        slippageTolerance: new Percent(parseNumber(tradeSettings.slippageTolerancePercentage, 2).toString(), 10000),
      };
    ...
    try {
      const approvalAmount: bigint =
        tradeType === "exactInput"
          ...
          : increaseByPercent(amountInWithDecimals, tradeOptions.slippageTolerance);
          ...
          <div className="w-[320px]">
            {status === "ready" && (
              <div className="flex flex-col items-center text-center">
                ...
                <div className="text-xl mb-2 font-semibold">Approve token spending</div>
                <div className="text-sm mb-4">
                  Before swapping, you need to approve token spending, which allows the platform to access your tokens.
                  This step doesn't swap your tokens yet; you'll still need to confirm the swap afterward.
                </div>
            ...
            {status === "awaiting_approval_confirmation" && (
              <div className="flex flex-col items-center text-center">
                <CircleNotch size={64} className="animate-spin" />
                <div className="text-xl mb-2 font-semibold">Awaiting transaction confirmation</div>
                <div className="text-sm mb-4">
                  We are waiting for the transaction to be confirmed on chain before proceeding
                </div>
            ...
            {status === "confirm_swap" && (
              <div className="flex flex-col items-center text-center">
                ...
                <div className="text-sm mb-4">Confirm the swap in your wallet</div>
                <div className="font-semibold">Proceed in your wallet</div>
              </div>
            )}
            {status === "error" && (
              <div className="flex flex-col items-center text-center">
                ...
                <div className="text-xl mb-2 font-semibold text-red-500">Error</div>
                <div className="text-sm mb-4">An error occurred while processing the transaction</div>
              </div>
            )}
            {status === "done" && (
              <div className="flex flex-col items-center text-center">
                <Cube size={64} />
                <div className="text-xl mb-2 font-semibold">Swap successful</div>
                <div className="text-sm mb-4">The swap was successful. You can view the transaction on CeloScan:</div>
                {transactionHash && (
                  <a
