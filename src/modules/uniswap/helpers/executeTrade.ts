import { ChainId, type Token, TradeType } from "@uniswap/sdk-core";
import { type Route, type SwapOptions, SwapRouter } from "@uniswap/v3-sdk";
import { getChainId, sendTransaction, simulateContract, writeContract } from "@wagmi/core";
import type { Address } from "viem";
import { ABI_CELO_ROUTER, CELO_SWAP_ROUTER_ADDRESS, MAINNET_SWAP_ROUTER_ADDRESS, getQuoteFromQuoter } from "../.";
import { decreaseByPercent, increaseByPercent } from "../../../shared/helpers/percent";
  ...
  if (chainId === ChainId.CELO) {
    try {
      ...
      if (type === "exactInput") {
        ...
        const amountOutMinimum = decreaseByPercent(BigInt(quotedAmountOut), options.slippageTolerance);
        ...
        const amountInMaximum = increaseByPercent(BigInt(quotedAmountIn), options.slippageTolerance);
