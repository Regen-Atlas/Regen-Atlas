...
export async function fetchGlwPrice(): Promise<TokenPrice | null> {
  ...
  try {
    ...
    let price = Number.parseFloat(pair.priceUsd || "0");
    if (price <= 0) {
    ...
    const marketCap = pair.marketCap
      ? Number.parseFloat(String(pair.marketCap))
      : pair.fdv
        ? Number.parseFloat(String(pair.fdv))
...
export async function fetchRegenSellOrders(
  ...
  try {
    ...
    for (const order of orders) {
      const askAmount = Number.parseFloat(order.ask_amount || "0");
      if (askAmount > 0 && askAmount < lowestAsk) {
        lowestAsk = askAmount;
      }
    }

    if (lowestAsk === Number.POSITIVE_INFINITY) return null;
...
export async function fetchAllPrices(): Promise<AllPrices> {
  const [char, glw] = await Promise.all([fetchCharPrice(), fetchGlwPrice()]);
