...
import type { SourceProtocol, VerifiableProvenance } from "../types";
...
async function fetchAllActions(): Promise<Action[]> {
  ...
  const { data, error } = await supabase.from("actions_published_view").select();
 ...
 */
export async function fetchActionsByProtocol(targetProtocol: SourceProtocol): Promise<VerifiableProvenance[]> {
  ...
  for (const action of actions) {
    ...
    const documentCIDs = action.proofs?.filter((p) => p.proof_link).map((p) => p.proof_link) ?? [];

    const certNames =
      action.certifications?.map((c) => c.certifier?.short_name ?? c.certifier?.name).filter(Boolean) ?? [];
