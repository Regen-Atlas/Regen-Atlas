import type { RegenBatch, RegenClassInfo, RegenProject } from "../types";
...
export async function fetchRegenBatches(projectId?: string): Promise<RegenBatch[]> {
  const allBatches: RegenBatch[] = [];
  let nextKey: string | null = null;

  // Use project-specific endpoint when projectId is provided
  const basePath = projectId ? `/regen/ecocredit/v1/batches-by-project/${projectId}` : "/regen/ecocredit/v1/batches";

  let pages = 0;
  const maxPages = 50; // safety limit against pagination loops
  do {
    const reqPath: string = nextKey ? `${basePath}?pagination.key=${encodeURIComponent(nextKey)}` : basePath;
    ...
    if (pages >= maxPages) {
      console.warn(
        `[Regen] Pagination safety limit hit for ${basePath} after ${pages} pages, ${allBatches.length} batches`,
      );
      break;
    }
  } while (nextKey);
  ...
  for (const batch of validBatches) {
    try {
      ...
      const tradable = Number.parseFloat(supply.tradable_amount || "0");
      const retired = Number.parseFloat(supply.retired_amount || "0");
...
export async function fetchProjectMetadata(metadataUri: string): Promise<Record<string, unknown>> {
...
export function getRegenSourceMeta(path: string, params: Record<string, string> = {}) {
  return {
