import type { ToucanProject, ToucanTCO2Token } from "../types";
...
const TOUCAN_POLYGON_SUBGRAPH_ID = "FU5APMSSCqcRy9jy56aXJiGV3PQmFQHg2tzukvSJBgwW";
...
async function querySubgraph<T>(query: string): Promise<T> {
  if (!TOUCAN_SUBGRAPH_URL) {
    throw new Error("Toucan requires VITE_GRAPH_API_KEY (free at thegraph.com/studio)");
  }
...
export async function fetchTCO2Tokens(first = 100, skip = 0): Promise<ToucanTCO2Token[]> {
  const query = `{
...
export async function fetchToucanProjects(first = 100): Promise<ToucanProject[]> {
  const query = `{
...
export async function fetchPooledTCO2Tokens(poolAddress: string, first = 1000): Promise<PooledTCO2[]> {
  const query = `{
    pooledTCO2Tokens(
...
export async function fetchToucanAggregations(): Promise<Array<{ id: string; key: string; value: string }>> {
  const query = `{
    aggregations {
      id
      key
...
export async function fetchPoolSummary(poolName: string, poolAddress: string): Promise<PoolSummary> {
