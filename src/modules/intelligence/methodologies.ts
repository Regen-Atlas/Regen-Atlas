import type { ConfidenceLevel, MethodologyCitation, MethodologyTier } from "./types";
...
const PROJECT_METHODOLOGIES: ProjectMethodology[] = [
  {
    ...
    citations: [
      {
        name: "Regen CarbonPlus Grasslands Methodology v1.0",
        url: "https://library.regen.network/v/methodology-library/carbonplus-grasslands",
        year: 2022,
      },
  ...
  },
  {
    ...
    citations: [
      ...
      {
        name: "EPA Social Cost of Carbon",
...
const BIOME_METHODOLOGIES: Record<string, BiomeMethodology> = {
  "tropical-forest": {
    ...
    citations: [
      {
        name: "Costanza et al. Changes in Global Value of Ecosystem Services",
  ...
  mangrove: {
    ...
    citations: [
      {
        name: "de Groot et al. Global Estimates of the Value of Ecosystems",
  ...
  default: {
    ...
    citations: [{ name: "TEEB Synthesis Report", year: 2010 }],
...
export function findProjectMethodology(
  ...
  const candidates = PROJECT_METHODOLOGIES.filter((m) => m.creditType === creditType);
  ...
  if (creditType === "BT" && normalizedBiome) {
    ...
    const match = candidates.find((m) => (isTropical ? m.id === "BT01" : m.id === "BT02"));
