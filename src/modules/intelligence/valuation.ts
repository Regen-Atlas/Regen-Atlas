...
import type { ConfidenceLevel, MethodologyTrace, ValuationMethodology, VerifiableProvenance } from "./types";
...
const TEEB_BIOME_VALUES: Record<string, { low: number; high: number; source: string }> = {
  "tropical-forest": {
    low: 5382,
    high: 16400,
...
export function valuateCarbon(
  ...
  const trace: MethodologyTrace = {
    ...
    citations: [
      {
        name: "EPA Technical Support Document: Social Cost of Greenhouse Gases",
...
export function valuateBiodiversity(hectares: number, biome: string): VerifiableProvenance["valuation"] {
  const normalizedBiome = biome.toLowerCase().replace(/\s+/g, "-");
  const biomeValues = TEEB_BIOME_VALUES[normalizedBiome] ?? TEEB_BIOME_VALUES.default;
  ...
  const trace: MethodologyTrace = {
    ...
    inputs: [
      ...
      {
        label: "Value Range",
        value: `$${biomeValues.low.toLocaleString()}-$${biomeValues.high.toLocaleString()}/ha/yr`,
        source: biomeMeta.citations[0]?.name,
      },
    ],
...
export function valuateRenewableEnergy(
  ...
  const emissionFactor = GRID_EMISSION_FACTORS[region] ?? GRID_EMISSION_FACTORS.default;
...
export function valuateMarineStewardship(hectares: number): VerifiableProvenance["valuation"] {
  ...
  const trace: MethodologyTrace = {
    ...
    inputs: [
      ...
      {
        label: "Value Range",
        value: `$${values.low.toLocaleString()}-$${values.high.toLocaleString()}/ha/yr`,
        source: "Barbier 2011",
      },
    ],
    citations: [
      {
        name: "Barbier et al. The Value of Estuarine and Coastal Ecosystem Services",
        url: "https://doi.org/10.1016/j.ecolecon.2010.09.024",
        year: 2011,
      },
    ],
...
export function valuateProjectSpecific(
  ...
  const trace: MethodologyTrace = {
    ...
    inputs: [
      ...
      {
        label: "Value Range",
        value: `$${methodology.valuePerUnit.low}-$${methodology.valuePerUnit.high}/${methodology.valuePerUnit.unit}`,
      },
    ],
