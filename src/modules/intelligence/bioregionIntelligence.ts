...
export function mapAssetsToBioregions(assets: Asset[], geojson: GeoJSON.FeatureCollection): Map<string, Asset[]> {
...
export function getBioregionStats(
  ...
  const feature = geojson.features.find((f) => f.properties?.code === bioregionCode);
...
export async function getBioregionForAsset(asset: Asset): Promise<BioregionProperties | null> {
  const geojson = await loadBioregionGeoJSON();
  const feature = findBioregionForPoint(asset.coordinates.longitude, asset.coordinates.latitude, geojson);
...
export function getOrgsBioregion(orgs: Org[], bioregionCode: string, geojson: GeoJSON.FeatureCollection): Org[] {
  const feature = geojson.features.find((f) => f.properties?.code === bioregionCode);
  if (!feature) return [];
...
export function getAllBioregionSummaries(
  ...
  const summaries = new Map<string, { code: string; assetCount: number; realm_name: string }>();
