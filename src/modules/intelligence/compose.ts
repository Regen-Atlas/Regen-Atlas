...
const REGEN_CREDIT_TYPE_MAP: Record<string, "climate" | "biodiversity" | "marine"> = {
  C: "climate",
  BT: "biodiversity",
  MBS: "marine",
};

export function composeToucanPoolProvenance(pool: {
  name: string;
  address: string;
  totalTCO2e: number;
  tokenCount: number;
  ...
  return {
    ...
    schemaVersion: "1.0",
    source: getToucanSourceMeta(`https://app.toucan.earth/pools/${pool.name.toLowerCase()}`, {
...
export function composeToucanProvenance(token: ToucanTCO2Token, totalSupply?: number): VerifiableProvenance {
  ...
  const vintage = formatVintage(token.projectVintage.startTime, token.projectVintage.endTime);

  return {
    attestor: "Regen Atlas",
    attestedAt: new Date().toISOString(),
    schemaVersion: "1.0",
    source: getToucanSourceMeta("https://api.thegraph.com/subgraphs/name/toucanprotocol/matic", { tokenId: token.id }),
    asset: {
      type: "Carbon Credit",
      subtype: standard,
      name: token.name,
...
export function composeRegenProvenance(
  ...
  const totalAmount = batches.reduce((sum, b) => sum + Number.parseFloat(b.total_amount || "0"), 0);

  // DEBUG: trace credit volumes per project (remove after diagnosis)
  console.log(
    `[Regen] ${project.id}: ${batches.length} batches, totalAmount=${totalAmount.toFixed(2)}, class=${project.class_id}`,
  );
  ...
  const geoJson = metadata?.["regen:projectLocation"] as { coordinates?: [number, number] } | undefined;
  const lat = geoJson?.coordinates?.[1] ?? 0;
  const lng = geoJson?.coordinates?.[0] ?? 0;
  const jurisdiction = (metadata?.["regen:projectJurisdiction"] as string) ?? project.jurisdiction ?? "Unknown";
  ...
  if (impactCategory === "climate") {
    impact.metrics.climate = {
      tCO2e: totalAmount,
      methodology: (metadata?.["regen:approvedMethodologies"] as string) ?? "Regen Network",
      vintage: batches[0] ? `${batches[0].start_date}-${batches[0].end_date}` : "Unknown",
    ...
    const biome = (metadata?.["regen:biomeType"] as string) ?? "temperate-forest";
  ...
  return {
    ...
    asset: {
      ...
      name: (metadata?.["schema:name"] as string) ?? `Regen Project ${project.id}`,
      chain: "regen-1",
    },
    impact,
    valuation,
    origin: {
      ...
      methodology: (metadata?.["regen:approvedMethodologies"] as string) ?? undefined,
...
export function composeGlowProvenance(reports: GlowWeeklyReport[]): VerifiableProvenance {
...
export function composeGlowFarmProvenance(farm: GlowAuditFarm, _weeksCovered: number): VerifiableProvenance {
  ...
  const annualMwh = farm.weeklyPower ? farm.weeklyPower * 52 : (farm.systemWattage * 4.5 * 365) / 1000; // avg 4.5 sun hours/day
...
export function aggregateImpact(provenances: VerifiableProvenance[]): ImpactAggregate {
  ...
  for (const p of provenances) {
    ...
    if (p.impact.metrics.biodiversity) {
      aggregate.totalBiodiversity.hectares += p.impact.metrics.biodiversity.hectares;
      aggregate.totalBiodiversity.projectCount++;
    }
    if (p.impact.metrics.energy && !isGlowFarm) {
      aggregate.totalEnergy.mwhGenerated += p.impact.metrics.energy.mwhGenerated;
      aggregate.totalEnergy.farmCount = Math.max(aggregate.totalEnergy.farmCount, p.impact.metrics.energy.farmCount);
    }
  ...
  for (const [proto, v] of Object.entries(protoValues)) {
    console.log(
      `[Aggregate] ${proto}: ${v.count} items, $${(v.low / 1e6).toFixed(1)}M - $${(v.high / 1e6).toFixed(1)}M`,
    );
  }
  console.log(
    `[Aggregate] TOTAL: $${(aggregate.totalValueUSD.low / 1e6).toFixed(1)}M - $${(aggregate.totalValueUSD.high / 1e6).toFixed(1)}M`,
  );
  ...
  for (const p of provenances) {
    ...
    if (p.valuation.tokenMarketContext) {
      ...
      if (mechanismType === "protocol-equity" && ctx.marketCap > 0) {
        ...
        const qty =
          p.impact.metrics.climate?.tCO2e ??
