...
import { AGENT_TYPE_EMOJIS, AGENT_TYPE_LABELS } from "../types";
...
export function AgentBadge({ type, size = "md", showLabel = true }: Props) {
  ...
  return (
    <span
      className={clsx("inline-flex items-center gap-1 rounded-full font-medium", typeColors[type], sizeClasses[size])}
    >
      <span>{emoji}</span>
