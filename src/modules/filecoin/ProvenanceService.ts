...
import { clearSupabaseActionsCache, fetchActionsByProtocol } from "../intelligence/sources/supabaseActions";
import type { AssetActionClass, ImpactAggregate, MechanismType, VerifiableProvenance } from "../intelligence/types";
...
function persistToStorage(): void {
  try {
    ...
    console.log(`[ProvenanceService] Persisted ${provs.length} provenances, ${assetProvenanceMap.size} asset mappings`);
  } catch (e) {
...
async function generateLocalCID(data: VerifiableProvenance): Promise<string> {
...
function isTypeCompatible(provenanceType: string, registryTypes: string[], registrySubtypes: string[]): boolean {
  ...
  if (pt.includes("carbon")) {
    ...
    return rts.includes("ecotokens") && subs.some((s) => s.includes("carbon") || s.includes("soil"));
  }
  ...
  if (pt.includes("marine")) {
    return rts.includes("ecotokens") && subs.some((s) => s.includes("water") || s.includes("biodiversity"));
  }
  return false;
}
...
function geoDistSq(lat1: number, lng1: number, lat2: number, lng2: number): number {
...
function filterTypeCompatible(provenanceType: string, candidates: Asset[]): Asset[] {
...
function matchProvenance(provenance: VerifiableProvenance, assets: Asset[]): Asset | null {
  ...
  if (expectedIssuer) {
    const issuerAssets = assets.filter((a) => a.issuer?.name === expectedIssuer);

    if (issuerAssets.length > 0) {
      // 2a. Coordinate proximity (within ~3.5km — tight to avoid default coords)
      if (provenance.origin.location.lat !== 0 || provenance.origin.location.lng !== 0) {
      ...
      const pName = provenance.asset.name.toLowerCase().replace(/[^a-z0-9]/g, "");
      for (const asset of issuerAssets) {
      ...
      const jurisdiction = provenance.origin.location.jurisdiction?.toLowerCase() ?? "";
      if (jurisdiction && jurisdiction !== "global" && jurisdiction !== "unknown") {
        ...
        const compatible = filterTypeCompatible(provenance.asset.type, countryAssets);
      ...
      const compatible = filterTypeCompatible(provenance.asset.type, issuerAssets);
      if (compatible.length > 0) {
        return hashPick(compatible, pKey);
      }
  ...
  for (const asset of assets) {
    ...
    if (isTypeCompatible(provenance.asset.type, rTypes, rSubs)) {
      if (provenance.origin.location.lat !== 0 && asset.coordinates?.latitude) {
        const dist = geoDistSq(
          provenance.origin.location.lat,
          provenance.origin.location.lng,
          asset.coordinates.latitude,
...
export async function ingestAllSources(
  ...
  const updateProgress = (idx: number, update: Partial<IngestProgress>) => {
    progress[idx] = { ...progress[idx], ...update };
    onProgress?.([...progress]);
  };

  const addProvenance = async (p: VerifiableProvenance, sourceIdx: number) => {
  ...
  try {
    ...
    console.log(
      "[Ingest] Prices loaded:",
      prices.char ? `CHAR=$${prices.char.price}` : "no CHAR",
      prices.glw ? `GLW=$${prices.glw.price}` : "no GLW",
    );
  } catch (e) {
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      const vintageQty = Number.parseFloat(rawQty) / 1e18;
      ...
      if (prices.char) {
        prov.valuation = addDirectCreditGap(prov.valuation, prices.char.price, prices.char.source);
      }
  ...
  try {
    ...
    const [projects, classes] = await Promise.all([fetchRegenProjects(), fetchRegenClasses()]);
    ...
    for (let i = 0; i < projectSlice.length; i += batchSize) {
      ...
      const results = await Promise.allSettled(
        chunk.map(async (project) => {
          ...
          const hasTradable = batches.some((b) => b._supply && b._supply.tradable > 0);

          if (hasTradable) {
            // Tradable credits exist — try to find sell orders for pricing
            stampMechanism(prov, "direct-credit", "asset");
            const tradableBatch = batches.find((b) => b._supply && b._supply.tradable > 0);
            if (tradableBatch) {
              try {
                const sellData = await fetchRegenSellOrders(tradableBatch.denom);
                if (sellData) {
                  prov.valuation = addDirectCreditGap(prov.valuation, sellData.lowestAskUSD, "regen-marketplace");
                }
              } catch {
    ...
    updateProgress(1, {
      status: "error",
      error: err instanceof Error ? err.message : "Regen Network fetch failed",
    });
  }
  ...
  try {
    ...
    const [reports, auditFarms] = await Promise.all([fetchGlowRecentReports(8), fetchGlowAuditFarms()]);
    ...
    if (reports.length > 0) {
      ...
      if (prices.glw?.price && prices.glw.marketCap) {
        glowProv.valuation = addGapFactor(scaledValuation, prices.glw.marketCap, prices.glw.price, prices.glw.source);
      } else {
...
export function getProvenanceByCid(cid: string): VerifiableProvenance | undefined {
...
export function getProvenancesForAsset(assetId: string): VerifiableProvenance[] {
  const cids = assetProvenanceMap.get(assetId) ?? [];
  return cids.map((cid) => provenanceCache.get(cid)).filter((p): p is VerifiableProvenance => !!p);
}
...
export function getAggregateImpact(provenances?: VerifiableProvenance[]): ImpactAggregate {
...
export async function batchUploadToFilecoin(
  ...
  uploadFn: (p: VerifiableProvenance) => Promise<{ pieceCid: string; size: number } | null>,
