...
function ChainRailItem({ chain }: { chain: { id: string; name: string } }) {
  return (
    <div className="flex items-center gap-2 px-5 shrink-0">
      <ChainIcon chainId={chain.id} chainName={chain.name} size={18} className="bg-gray-400" />
      <span className="text-[11px] text-gray-500 whitespace-nowrap">{chain.name}</span>
    </div>
  );
}
              ...
              <div className="grid lg:grid-cols-2 gap-5">
                <div className="bg-white border border-gray-200 p-7 flex flex-col hover:shadow-[0_8px_30px_rgba(94,173,185,0.1)] hover:border-primary-200/60 transition-all duration-300">
                  ...
                  <p className="text-sm text-gray-500 leading-[1.7] flex-1">
                    Browse assets, organizations, and actions across 185 bioregions. Filter by chain, ecosystem service,
                    issuer, or location. Every entity geolocated to its bioregion.
                  </p>
                  <Link
                    to="/"
                ...
                <div className="bg-white border border-gray-200 p-7 flex flex-col hover:shadow-[0_8px_30px_rgba(94,173,185,0.1)] hover:border-primary-200/60 transition-all duration-300 relative overflow-hidden">
                  <span className="absolute top-3 right-3 text-[9px] font-bold uppercase tracking-widest text-primary-300 bg-primary-50 px-2 py-0.5 rounded-full">
                    Coming Soon
                  </span>
                  <Database size={28} weight="duotone" className="text-primary-300 mb-4" />
                  <h3 className="text-base font-semibold mb-2">Impact Intelligence</h3>
                  <p className="text-sm text-gray-500 leading-[1.7] flex-1">
                    Track where green assets come from and how they perform. Verified data from every major tokenization
                    protocol, benchmarked against peer-reviewed science.
                  </p>
                  <a
