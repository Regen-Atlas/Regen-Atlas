...
export default ({ className, org, selectClicked }: OrgCardProps): React.ReactElement => {
    ...
    <>
      <div className={clsx("asset-card border-2 border-white p-3 rounded-[20px] bg-cardBackground", className)}>
          ...
          <div className="flex gap-3 justify-between items-center">
            <div className="flex gap-3">
              {org.ecosystems.map((ecosystem) => (
                ...
                >
                  <img src={ecosystem.icon} alt={ecosystem.name} className="w-7 h-7 rounded-full" />
                </div>
              ))}
            </div>
            <div className="flex gap-3">
        ...
        <div className="lg:hidden">
          <ExpandableText text={org.description} maxChars={125} className="text-sm mb-3" />
        </div>
        <div className="hidden lg:block">
          <ExpandableText text={org.description} maxChars={150} className="text-sm mb-3" />
        </div>
              ...
              <div className="xxs:text-xs text-sm font-bold text-right flex items-center flex-wrap gap-2">
                ...
                >
                  <div className="bg-grayTag h-7 flex justify-center items-center rounded-full px-4 xxs:text-xs text-sm font-bold cursor-pointer">
                    {org.assets[0].name.length > 20 ? `${org.assets[0].name.substring(0, 17)}...` : org.assets[0].name}
                  </div>
                </Link>
                <div
                  className="relative"
              ...
              <div className="xxs:text-xs text-sm font-bold text-right flex items-center gap-2">
                {org?.treasury?.map((treasury) => (
                  <a
                    ...
                    rel="noreferrer"
                  >
                    <ChainTag key={treasury.platform.id} platform={treasury.platform} />
                  </a>
                ))}
              </div>
            </div>
