...
export function formatNumber(value: bigint, decimals = 18, displayDecimals = 5): string {
  const number = formatUnits(value, decimals);
  return Number.parseFloat(number).toFixed(displayDecimals);
}
...
export function divideBigInts(a: bigint, b: bigint, precision = 2): string {
  ...
  const decimalPart = decimalIndex > 0 ? resultStr.slice(decimalIndex) : resultStr.padStart(precision, "0");
