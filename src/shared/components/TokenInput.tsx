    ...
    <div className="grid p-4 rounded-lg border-2 border-gray-400 bg-cardBackground">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex-shrink-0">
          {token.symbol && <TokenSelect selectedToken={token} onTokenChange={(token) => onTokenChange(token)} />}
        </div>
      </div>
      {displayBalance && <div className="text-xs text-right mt-1">Balance: {formattedBalance}</div>}
    </div>
  );
};
