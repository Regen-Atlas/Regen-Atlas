...
export function BioregionLayer({
  ...
  const combinedCountMap = useMemo(() => {
    if (!geojson)
      return new Map<string, { total: number; assets: number; actors: number; agents: number; actions: number }>();
    const counts = new Map<
      string,
      { total: number; assets: number; actors: number; agents: number; actions: number }
    >();
    ...
    const features = geojson.features
      ...
      .map((f, i) => {
        ...
        return {
          ...
          geometry: {
            type: "Point" as const,
            coordinates: typeof centroid === "string" ? JSON.parse(centroid) : centroid,
          },
        };
      });
  ...
  const selectedGeojson = useMemo(() => {
    ...
    const feature = geojson.features.find((f) => f.properties?.code === selectedBioregion);
    return {
  ...
  const handleClick = useCallback(
    (e: MapMouseEvent) => {
      ...
      const entityLayers = [
        "unclustered-point",
        "clusters", // Assets
        "org-unclustered-point",
        "org-clusters", // Orgs
        "unclustered-action-point",
        "action-clusters", // Actions
      ].filter((id) => {
        try {
      ...
      const centroid = props.centroid ? JSON.parse(props.centroid) : [0, 0];
      onBioregionSelect({
        code: props.code,
  ...
  const handleMouseMove = useCallback(
    (e: MapMouseEvent) => {
      ...
      if (hoveredIdRef.current !== null) {
        map.setFeatureState({ source: "bioregions", id: hoveredIdRef.current }, { hover: false });
      ...
      if (feature.id != null) {
        ...
        map.setFeatureState({ source: "bioregions", id: feature.id }, { hover: true });
      }
      map.getCanvas().style.cursor = "pointer";

      const props = feature.properties as Record<string, any>;
      const counts = combinedCountMapRef.current.get(props.code) ?? {
        total: 0,
        assets: 0,
        actors: 0,
        agents: 0,
  ...
  return (
    <>
      {/* ── Bioregion polygons ── */}
      <Source id="bioregions" type="geojson" data={geojson} generateId={true}>
        <Layer
          id="bioregion-fill"
          type="fill"
          source="bioregions"
        ...
        <Layer
          ...
          layout={{ visibility }}
          paint={{
            "line-color": ["case", ["boolean", ["feature-state", "hover"], false], ["get", "color"], "#D1D5DB"],
            "line-width": [
              "interpolate",
              ["linear"],
              ["zoom"],
      ...
      <Source id="bioregion-labels" type="geojson" data={labelCentroidGeojson as any}>
        <Layer
          id="bioregion-label"
          type="symbol"
          ...
          layout={{
            ...
            "text-field": ["step", ["zoom"], "", 4, ["get", "code"], 5.5, ["get", "name"]],
            "text-size": ["interpolate", ["linear"], ["zoom"], 4, 8, 6, 10, 8, 12],
          ...
          paint={{
            ...
            "text-halo-width": 1.2,
            "text-opacity": ["interpolate", ["linear"], ["zoom"], 4, 0.4, 6, 0.65],
          }}
        />
      </Source>

      {/* ── Bioregion count badges — hollow ring style (show when zoomed out) ── */}
      <Source id="bioregion-counts" type="geojson" data={centroidGeojson as any}>
        ...
        <Layer
          ...
          maxzoom={4}
          paint={{
            "circle-radius": ["interpolate", ["linear"], ["get", "count"], 1, 12, 10, 16, 50, 22],
        ...
        <Layer
          ...
          layout={{
            ...
            "text-size": ["interpolate", ["linear"], ["get", "count"], 1, 10, 10, 12, 50, 14],
      ...
      <Source id="bioregion-selected" type="geojson" data={selectedGeojson as any}>
        <Layer
      ...
      <Source id="bioregion-selected-label-src" type="geojson" data={selectedLabelGeojson as any}>
        <Layer
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200 max-w-[260px] overflow-hidden">
            <div className="flex gap-2.5 p-2.5">
              ...
              <div className="min-w-0">
                <div className="text-sm font-semibold text-gray-900 leading-tight">{hoverInfo.name}</div>
                <div className="mt-1">
                  <span
            ...
            <div className="flex items-center gap-3 px-2.5 py-2 border-t border-gray-100">
              <div className="flex items-center gap-1 text-[11px] text-gray-500" title="Assets">
                <svg width="11" height="11" viewBox="0 0 256 256" className="fill-current">
                  <path d="M128,24A104,104,0,1,0,232,128,104.11,104.11,0,0,0,128,24Zm0,192a88,88,0,1,1,88-88A88.1,88.1,0,0,1,128,216ZM173.66,90.34a8,8,0,0,1,0,11.32l-40,40a8,8,0,0,1-11.32-11.32l40-40A8,8,0,0,1,173.66,90.34ZM96,16V8a8,8,0,0,1,16,0v8a8,8,0,0,1-16,0Z" />
                </svg>
