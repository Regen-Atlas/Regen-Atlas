...
function generateClusterIcon(assetCount: number, actorCount: number, actionCount: number): HTMLCanvasElement {
  ...
  segments.forEach((seg, i) => {
    ...
    const text = seg.count >= 1000 ? `${Math.floor(seg.count / 1000)}k` : String(seg.count);
...
function spiderfyFeatures(items: SpiderfyItem[], map: MapRef, pixelRadius = 20): SpiderfyItem[] {
...
export function CompositeClusterLayer({
  ...
  useEffect(() => {
    ...
    const handleClick = (e: MapMouseEvent) => {
      ...
      if (feature.layer?.id === CLUSTER_LAYER && feature.geometry?.type === "Point") {
    ...
    const handleMouseMove = (e: MapMouseEvent) => {
      ...
      if (!isMobile && feature.layer?.id === UNCLUSTERED_POINT) {
        ...
        if (entityType === "asset" && props) {
          const asset = assetById.get(props.id);
        ...
        } else if (entityType === "action" && props) {
          const action = actionById.get(props.id);
  ...
  const pointFillColor: mapboxgl.Expression = [
    ...
    "#ffffff",
    "actor",
    ENTITY_COLORS.actor.primary,
    "action",
    ENTITY_COLORS.action.primary,
    "#ffffff",
  ];
  ...
  const glowColor: mapboxgl.Expression = [
    ...
    "actor",
    ENTITY_COLORS.actor.light,
    "action",
    ENTITY_COLORS.action.light,
    "#BDBDBD",
  ];
    ...
    >
      ...
      <Layer
        ...
        paint={{
          ...
          "circle-radius": ["step", ["get", "point_count"], 14, 10, 17, 50, 20, 100, 24],
          "circle-stroke-color": [
            "case",
            ["boolean", ["feature-state", "hover"], false],
            "rgba(34,211,238,0.6)",
      ...
      <Layer
        ...
        filter={["!", ["has", "point_count"]]}
        paint={{
          "circle-radius": ["case", ["boolean", ["feature-state", "hover"], false], 18, isMobile ? 20 : 14],
      ...
      <Layer
        ...
        paint={{
          ...
          "circle-radius": ["case", ["boolean", ["feature-state", "hover"], false], 8, isMobile ? 8 : 6],
          "circle-stroke-width": ["case", ["boolean", ["feature-state", "hover"], false], 3, 2.5],
        ...
        >
          <div className="flex items-center gap-2 max-w-[200px]">
            {hoveredAsset.imageUrl && (
              <img src={hoveredAsset.imageUrl} alt="" className="w-10 h-10 rounded object-cover flex-shrink-0" />
            )}
            <div className="min-w-0">
              <div className="text-sm font-semibold text-gray-900 truncate">{hoveredAsset.name}</div>
              <div className="text-xs text-gray-500 truncate">{hoveredAsset.issuerName}</div>
              {hoveredAsset.assetTypeName && (
                <div className="text-[11px] text-gray-400 truncate">{hoveredAsset.assetTypeName}</div>
              )}
            </div>
          </div>
        </Popup>
        ...
        >
          <div className="flex items-center gap-2 max-w-[200px]">
            {hoveredAction.imageUrl && (
              <img src={hoveredAction.imageUrl} alt="" className="w-10 h-10 rounded object-cover flex-shrink-0" />
            )}
            <div className="min-w-0">
              <div className="text-sm font-semibold text-gray-900 truncate">{hoveredAction.name}</div>
              {hoveredAction.actorName && !hoveredAction.actorName.startsWith("0x") && (
                <div className="text-xs text-gray-500 truncate">{hoveredAction.actorName}</div>
              )}
              {hoveredAction.protocolName && (
                <div className="text-[11px] text-gray-400 truncate">{hoveredAction.protocolName}</div>
              )}
            </div>
          </div>
        </Popup>
