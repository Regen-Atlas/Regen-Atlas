import { GlobeHemisphereWest, MapTrifold, Mountains, Signpost } from "@phosphor-icons/react";
    ...
    <div className="flex items-center gap-0.5 bg-white/80 backdrop-blur-sm rounded-md p-0.5 shadow-sm">
      {STYLE_ICONS.map(({ key, icon: Icon, label }) => (
        <button
          key={key}
          title={label}
          onClick={() => dispatch({ type: "SET_MAP_STYLE", payload: key })}
          className={clsx(
            "w-7 h-7 flex items-center justify-center rounded cursor-pointer transition-all",
            mapStyle === key ? "bg-primary-100 text-gray-800" : "text-gray-500 hover:bg-gray-100 hover:text-gray-700",
          )}
        >
      ...
      <button
        ...
        onClick={() => dispatch({ type: "SET_SHOW_POLITICAL", payload: !showPolitical })}
        className={clsx(
          "w-7 h-7 flex items-center justify-center rounded cursor-pointer transition-all",
          showPolitical ? "bg-primary-100 text-gray-800" : "text-gray-500 hover:bg-gray-100 hover:text-gray-700",
        )}
      >
