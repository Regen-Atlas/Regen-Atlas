import { ArrowUpRight, CalendarBlank, Certificate, Export, MapPin, Users } from "@phosphor-icons/react";
...
export default ({ className, action, selectClicked }: ActionCardProps): React.ReactElement => {
  ...
  return (
    <>
      <div className={clsx("asset-card border-2 border-white p-3 rounded-[20px] bg-cardBackground", className)}>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 pb-2 text-sm">
          {(action.region || action.country_code) && (
            <div className="flex items-center font-bold cursor-pointer" onClick={selectClicked}>
              <MapPin size={16} className="mr-1" />
              {[action.region, action.country_code ? COUNTRY_CODE_TO_NAME[action.country_code] : null]
                .filter(Boolean)
                .join(", ")}
            </div>
          )}
        ...
        {action.description && (
          <>
            <div className="lg:hidden">
              <ExpandableText text={action.description} maxChars={125} className="text-sm mb-3" />
            </div>
            <div className="hidden lg:block">
              <ExpandableText text={action.description} maxChars={150} className="text-sm mb-3" />
            </div>
          </>
        )}
            ...
            <div className="flex justify-between items-start py-2 min-h-9">
              ...
              <div className="text-sm font-bold text-right flex items-center flex-wrap gap-2 justify-end">
                {action.actors.slice(0, 3).map((actor) => (
                  ...
                  >
                    {actor.name.length > 23 ? `${actor.name.substring(0, 20)}...` : actor.name}
                  </a>
                ))}
      ...
      {showShareOptions && <TextShareModal text={shareUrl} onClose={() => setShowShareOptions(false)} />}
    </>
  );
};
