...
type DetailSection = "issuer" | "ratings" | "chains" | "children" | "tokens" | "provenance" | "parents";
...
export function AssetBioregionCard({ asset, onAssetSelect, showExternalLinks }: AssetBioregionCardProps) {
  const { allAssets } = useNewFiltersState();
  const [openSections, setOpenSections] = useState<Set<DetailSection>>(
    new Set(["issuer", "ratings", "chains", "children"]),
  );

  const toggleSection = (id: string) => {
    const section = id as DetailSection;
  ...
  const location = [asset.region, COUNTRY_CODE_TO_NAME[asset.country_code]].filter(Boolean).join(", ");
  ...
  return (
    <div className="bg-cardBackground border border-gray-200 overflow-hidden flex flex-col h-full">
      ...
      <div className="relative overflow-hidden">
        ...
        <div
          className={`absolute inset-0 ${asset.main_image ? "bg-gradient-to-r from-black/70 via-black/50 to-black/30" : "bg-gray-800"}`}
        />
        <div className="relative z-10 px-5 py-4">
          <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
            ...
            {asset.asset_subtypes.map((s) => (
              <span key={s.id} className="text-[11px] font-medium px-1.5 py-0.5 rounded bg-white/20 text-white/90">
                {s.name}
              </span>
            ))}
          </div>
          ...
          <div className="flex items-center gap-1 mt-1.5 text-xs text-white/70">
            ...
            {location && asset.issuer?.name && <span className="text-white/40">·</span>}
            {asset.issuer?.name && <span className="font-medium text-white/90">{asset.issuer.name}</span>}
          </div>
        </div>
      </div>
              ...
              <div className="space-y-3">
                {asset.issuer && (
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-800">{asset.issuer.name}</span>
                    {asset.issuer_link && (
                      <a
                        href={asset.issuer_link}
                        target="_blank"
                ...
                {asset.platforms.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {asset.platforms.map((p) => (
                      <div key={p.id} className="flex items-center gap-1.5 bg-gray-50 px-2.5 py-1.5">
            ...
            >
              <div className="space-y-2">
                {asset.certifications.map((cert) => (
                  <div key={cert.id} className="flex items-start justify-between gap-3 bg-gray-50 px-3 py-2">
                    <div className="min-w-0">
                      <div className="text-xs font-semibold text-gray-800">
                        {cert.certifier.short_name || cert.certifier.name}
                      </div>
                      {cert.description_short && (
                        <div className="text-[11px] text-gray-500 mt-0.5">{cert.description_short}</div>
                      )}
                      {cert.description && cert.description !== cert.description_short && (
                        <div className="text-[11px] text-gray-400 mt-0.5">{cert.description}</div>
                      )}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {cert.value > 0 && <span className="text-xs font-bold text-gray-700">{cert.value}</span>}
            ...
            >
              <div className="space-y-1">
                {asset.tokens.map((token) => (
                  <div key={token.id} className="flex items-center justify-between text-xs bg-gray-50 px-3 py-2">
              ...
              >
                ...
                <div className="space-y-2">
                  {provs.map((p, i) => {
                    ...
                    return (
                      <div key={p.pieceCid ?? i} className="bg-gray-50 px-3 py-2.5 text-xs space-y-1.5">
                        {/* Source + pathway */}
                        <div className="flex items-center gap-1.5">
                          <span className="font-semibold capitalize text-gray-800">{p.source.protocol}</span>
                          ...
                          <div className="text-[11px] text-gray-500 space-y-0.5">
                            {p.origin.project && (
                              <div className="truncate">
                                <span className="text-gray-600 font-medium">Project:</span> {p.origin.project}
                              </div>
                            )}
                            {p.origin.methodology && (
                              <div className="truncate">
                                <span className="text-gray-600 font-medium">Method:</span> {p.origin.methodology}
                              </div>
                            )}
                          </div>
                        )}
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-gray-600">
                          {m.climate && (
                            <span>
                              ...
                              {m.climate.standard && <span className="text-gray-400 ml-1">({m.climate.standard})</span>}
                            </span>
                          )}
                          {m.biodiversity && (
                            <span>
                        ...
                        <div className="text-gray-500 pt-0.5 border-t border-gray-100">
                          ...
                          <span className="font-semibold text-gray-700">
                            ${p.valuation.totalValue.low.toLocaleString()} – $
                            {p.valuation.totalValue.high.toLocaleString()}
                          </span>
                          <span className="text-gray-400 ml-1">({p.valuation.methodology})</span>
                          {p.valuation.gapFactor && (
                            <div className="text-blue-600 font-medium mt-0.5">
                              Market gap: {p.valuation.gapFactor.low.toFixed(1)}x –{" "}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          {childIssuer && <span className="text-[10px] text-gray-400 truncate">{childIssuer}</span>}
                          {childTypes.length > 0 && childIssuer && <span className="text-gray-300 text-[10px]">·</span>}
                      ...
                      <ArrowRight size={12} className="flex-shrink-0 text-gray-300 group-hover:text-blue-500" />
                    </button>
                  );
                })}
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          {parentIssuer && <span className="text-[10px] text-gray-400 truncate">{parentIssuer}</span>}
