...
type DetailSection = "issuers" | "chains" | "links" | "assets";
...
export function OrgBioregionCard({ org }: OrgBioregionCardProps) {
  ...
  return (
    <div className="bg-cardBackground border border-gray-200 overflow-hidden flex flex-col h-full">
      ...
      <div className="relative overflow-hidden">
        ...
        <div
          className={`absolute inset-0 ${org.main_image ? "bg-gradient-to-r from-black/70 via-black/50 to-black/30" : "bg-gray-800"}`}
        />
        <div className="relative z-10 px-5 py-4">
          <div className="flex items-center gap-1.5 mb-1.5">
            <span
              className={`text-[11px] font-semibold px-1.5 py-0.5 rounded text-white ${(org as any).isAgent ? "bg-purple-500/30" : "bg-blue-500/30"}`}
            >
              {(org as any).isAgent ? "Agent" : "Organization"}
            </span>
          </div>
          <h3 className="text-base font-bold text-white leading-tight">{org.name}</h3>
          <div className="flex items-center gap-1 mt-1.5 text-xs text-white/70">
            ...
            {org.address && org.established && <span className="text-white/40">·</span>}
            ...
            >
              <div className="space-y-1">
                {org.assets.map((asset) => (
                  <div key={asset.id} className="flex items-center gap-2 bg-gray-50 px-3 py-2">
