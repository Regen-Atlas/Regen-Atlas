                ...
                <div className="flex flex-col h-full pb-24">
                      ...
                      return (
                        <div key={type.id}>
                          <TypeSummary
                              ...
                              if (!selected) {
                                dispatchFilters({
                                  ...
                                  payload: {
                                    id: type.id,
                                    name: type.name,
                                    subtypes: type.asset_subtypes.map((s) => s.id),
                                  },
                                });
                              } else {
                            ...
                            onToggleClick={(e) => {
                              e.stopPropagation();
                              setOpenType((prev) => (prev === type.id ? undefined : type.id));
                            }}
                            ...
                            <div className="pl-8">
                              {type.asset_subtypes.map((subtype) => (
                                <div
                                  ...
                                  onClick={() => handleSubtypeClick({ typeId: type.id, subtypeId: subtype.id })}
                                >
                                  <CheckboxBox
                                    className="flex-shrink-0"
                                    checked={filters.assetTypes[type.id]?.subtypes.includes(subtype.id)}
                                  />
                  ...
                  <div className="flex justify-end mt-auto">
                    <button
                      ...
                      onClick={() => setOpenFilters((p) => ({ ...p, type: false }))}
                    >
                      Done
                    </button>
                  </div>
                ...
                <div className="grid gap-4 pl-8">
                  {base.issuers.map((provider) => (
                    <div
                      ...
                      onClick={() => dispatchFilters({ type: "SET_PROVIDER_FILTER", payload: provider.id })}
                    >
                ...
                <div className="grid gap-4 pl-8">
                  {base.platforms.map((plat) => (
                    <div
                      key={plat.id}
                      className="flex items-center gap-2 cursor-pointer"
                      onClick={() => dispatchFilters({ type: "SET_PLATFORM_FILTER", payload: plat.id })}
                    >
      ...
      <div className="flex justify-between mt-auto pt-4">
        <button
          ...
          onClick={() => {
            ...
            if (onActionFiltersChange)
              onActionFiltersChange({ protocols: new Set(), sdgs: new Set(), timeRange: null });
          }}
        >
          Clear
        </button>
