import { ArrowRight, Globe, Lightning, MapPin, ShieldCheck, TreeStructure, Users } from "@phosphor-icons/react";
...
export function AssetExploreCard({ asset, onLocate }: AssetExploreCardProps) {
  ...
  if (asset.certifications.length > 0)
    signals.push(`${asset.certifications.length} rating${asset.certifications.length > 1 ? "s" : ""}`);
  if (asset.child_assets.length > 0) signals.push(`${asset.child_assets.length} derived`);
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0 px-2.5 py-2.5">
          ...
          <div className="flex items-center justify-between gap-2 mb-0.5">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {asset.platforms.slice(0, 2).map((p) => (
                <div
                  key={p.id}
                  className="w-4 h-4 flex items-center justify-center rounded-full [&>div]:!w-4 [&>div]:!h-4"
                >
                  <ChainTag platform={p} />
                </div>
              ))}
          ...
          <div className="flex items-center gap-1 mt-0.5 text-xs text-gray-500">
            ...
            {location && asset.issuer?.name && <span className="text-gray-300">·</span>}
            {asset.issuer?.name && <span className="font-medium text-gray-600 truncate">{asset.issuer.name}</span>}
          </div>
...
export function OrgExploreCard({ org, onLocate }: OrgExploreCardProps) {
  const signals: string[] = [];
  if (org.assets.length > 0) signals.push(`${org.assets.length} asset${org.assets.length > 1 ? "s" : ""}`);
  if (org.issuers.length > 0) signals.push(`${org.issuers.length} issuer${org.issuers.length > 1 ? "s" : ""}`);
    ...
    >
      <div className="flex">
        ...
        <div className="flex-1 min-w-0 px-2.5 py-2.5">
          ...
          <div className="flex items-center justify-between gap-2 mb-0.5">
            <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded bg-blue-50 text-blue-600">Org</span>
            <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <div className="flex items-center gap-1 mt-0.5 text-xs text-gray-500">
            ...
            {org.address && org.established && <span className="text-gray-300">·</span>}
            {org.established && <span>Est. {new Date(org.established).getFullYear()}</span>}
          </div>
          ...
          {signals.length > 0 && (
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              {signals.map((s) => (
                <span key={s} className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-blue-50 text-blue-600">
                  {s}
                </span>
              ))}
            </div>
...
export function ActionExploreCard({ action, onLocate }: ActionExploreCardProps) {
  const location = [action.region, action.country_code ? COUNTRY_CODE_TO_NAME[action.country_code] : null]
    .filter(Boolean)
    .join(", ");

  const sortedSdgs = [...action.sdg_outcomes].sort((a, b) => Number.parseInt(a.code, 10) - Number.parseInt(b.code, 10));
      ...
      <div className="flex">
        ...
        <div className="flex-1 min-w-0 px-2.5 py-2.5">
          ...
          <div className="flex items-center gap-1 mt-0.5 text-xs text-gray-500">
            ...
            {location && (action.action_start_date || action.created_at) && <span className="text-gray-300">·</span>}
...
export function BioregionExploreCard({ bioregion, onSelect }: BioregionExploreCardProps) {
  return (
    ...
    >
      <div className="flex">
        <div className="w-1 flex-shrink-0" style={{ backgroundColor: bioregion.color }} />
        ...
        <div className="flex-1 min-w-0 px-2.5 py-2.5">
          ...
          {/* Row 2: name */}
          <div className="flex items-center justify-between gap-2">
            <h4 className="text-sm font-semibold text-gray-900 truncate">{bioregion.name}</h4>
