...
import type { NewAssetTypeFilters, NewFilters } from "../../modules/filters";
...
const newReducer = (state: NewState, action: NewAction): NewState => {
  switch (action.type) {
      ...
      const filters = {
        ...
        assetTypes: {
          ...
          [action.payload.typeId]: {
            ...
            subtypes: [...(state.filters.assetTypes[action.payload.typeId]?.subtypes || []), action.payload.subtypeId],
          },
        },
      };
      return {
    ...
    case "REMOVE_SUBTYPE_FILTER": {
      const updatedSubtypes = state.filters.assetTypes[action.payload.typeId]?.subtypes.filter(
        (subtype) => subtype !== action.payload.subtypeId,
      );
    ...
    case "SET_SELECTED_ASSET": {
      ...
      const selectedAssetIndex = filteredAssets.findIndex((asset) => asset.id === action.payload);
    ...
    case "TOGGLE_ENTITY_TYPE": {
      ...
      const entityType: EntityType =
        next.size === ALL_ENTITY_TYPES.length ? "all" : next.size === 1 ? ([...next][0] as EntityType) : "all";
    ...
    case "TOGGLE_ACTOR_TYPE": {
      ...
      const actorSubfilter =
        next.has("orgs") && next.has("agents")
          ? "both"
          : next.has("orgs")
            ? "orgs"
            : next.has("agents")
...
const NewFiltersStateProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [state, dispatch] = useReducer(newReducer, newInitialState);

  return <NewFiltersStateContext.Provider value={{ state, dispatch }}>{children}</NewFiltersStateContext.Provider>;
};
