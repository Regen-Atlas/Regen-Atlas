...
import type { NewFilters } from "../../modules/filters";

export const filterNewAssets = (assets: Asset[], filters: NewFilters): Asset[] => {
  const {
  ...
  return assets.filter((asset) => {
    ...
    if (filterPlatforms.length > 0 && !platforms.some((platform) => filterPlatforms.includes(platform.id))) {
      return false;
    }

    if (filterAssetTypes && Object.keys(filterAssetTypes).length > 0) {
      // Check if the asset has any of the selected asset types
      const hasSelectedAssetType = filterAssetTypes[assetTypeId] || filterAssetTypes[assetTypeId2];
      ...
      if (filterAssetTypes[assetTypeId]) {
        filterAssetTypes[assetTypeId].subtypes.forEach((subtype) => selectedSubtypes.add(subtype));
      }
      if (filterAssetTypes[assetTypeId2]) {
        filterAssetTypes[assetTypeId2].subtypes.forEach((subtype) => selectedSubtypes.add(subtype));
      }

      // If subtypes are selected, check if the asset has any of them
      if (selectedSubtypes.size > 0) {
        const assetSubtypes = [assetSubtypeId, assetSubtypeId2, assetSubtypeId3].filter(Boolean);
        const hasMatchingSubtype = assetSubtypes.some((subtype) => selectedSubtypes.has(subtype));
    ...
    if (filterFlags) {
      ...
      if (filterFlags.prefinancing !== null && asset.prefinancing !== filterFlags.prefinancing) {
        return false;
      }

      // Check pretoken flag
      if (filterFlags.pretoken !== null && asset.pretoken !== filterFlags.pretoken) {
        return false;
      }

      // Check yield_bearing flag
      if (filterFlags.yield_bearing !== null && asset.yield_bearing !== filterFlags.yield_bearing) {
        return false;
      }
    }
